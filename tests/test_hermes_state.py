...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db, monkeypatch):
        ...
        @contextmanager
        def traced_read_ctx():
            # A WAL search can borrow a different pooled connection for each
            # statement. Trace the connection actually checked out instead of
            # pre-opening one that the search may never use.
