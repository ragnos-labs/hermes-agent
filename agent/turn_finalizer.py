...
def finalize_turn(
    ...
    *,
    ...
    budget_fallback_eligible = (
        budget_exhausted
        and not suppress_external_effects
        and not interrupted
        and not failed
    ...
    if final_response and not interrupted and not suppress_external_effects:
        try:
    ...
    _leftover_steer = (
        None if suppress_external_effects else agent._drain_pending_steer()
    )
    if _leftover_steer:
